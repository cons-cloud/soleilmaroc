...
import { Settings, Save, Globe, Mail, Phone, MapPin, Facebook, Instagram, Twitter, Clock } from 'lucide-react';
...
const SettingsManagement: React.FC = () => {
  ...
  const loadSettings = async () => {
    try {
      ...
      if (data && data.length > 0) {
        ...
        setSettings(prev => ({
          ...prev,
          contact_email: settingsMap.email || prev.contact_email,
          contact_phone: settingsMap.phone || prev.contact_phone,
          contact_address: settingsMap.address || prev.contact_address,
        }));
      }
    } catch (error) {
  ...
  const handleSave = async () => {
    try {
      ...
      // Update contact info in site_content
      const updates = [
        { key: 'email', value: settings.contact_email },
        { key: 'phone', value: settings.contact_phone },
        { key: 'address', value: settings.contact_address },
      ];
  ...
  return (
      ...
      <div className="space-y-6">
        ...
        <div className="bg-white/80 backdrop-blur-sm rounded-lg shadow-sm p-6">
          <div className="flex items-center mb-4">
            <Facebook className="h-5 w-5 text-emerald-600 mr-2" />
        ...
        <div className="bg-white/80 backdrop-blur-sm rounded-lg shadow-sm p-6">
          ...
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-sm text-gray-500">Désactiver temporairement le site public</p>
            </div>
            <label className="relative inline-flex items-center cursor-pointer">
              ...
              />
              <div className="w-11 h-6 bg-gray-200 peer-focus:outline-none peer-focus:ring-4 peer-focus:ring-emerald-300 rounded-full peer peer-checked:after:translate-x-full peer-checked:after:border-white after:content-[''] after:absolute after:top-[2px] after:left-[2px] after:bg-white/80 backdrop-blur-sm after:border-gray-300 after:border after:rounded-full after:h-5 after:w-5 after:transition-all peer-checked:bg-emerald-600"></div>
            </label>
          </div>
        </div>
      </div>
...
export { SettingsManagement as default };
