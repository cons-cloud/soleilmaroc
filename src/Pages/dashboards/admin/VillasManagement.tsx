...
const VillasManagement: React.FC = () => {
  ...
  const [villaToDelete, setVillaToDelete] = useState<any>(null);
  ...
  const handleDeleteClick = (villa: any) => {
    ...
    <div className="space-y-6">
        <div className="flex justify-between items-center">
          <div>
            ...
            <p className="text-gray-600 mt-1">{filteredVillas.length} villa(s)</p>
          </div>
            ...
            <div key={item.id} className="bg-white/80 backdrop-blur-sm rounded-lg shadow-sm overflow-hidden hover:shadow-md transition-shadow">
              ...
              <div className="p-4">
                ...
                <div className="flex items-center gap-3 text-sm text-gray-600 mb-3">
                  ...
                  {item.has_pool && <span>🏊 Piscine</span>}
                </div>
        ...
        {filteredVillas.length === 0 && (
          <div className="bg-white/80 backdrop-blur-sm rounded-lg shadow-sm p-12 text-center">
            ...
            <h3 className="text-lg font-medium text-gray-900 mb-2">Aucune villa trouvée</h3>
          </div>
        )}
      ...
      {showConfirm && (
        <ConfirmDialog
          ...
          message={`Êtes-vous sûr de vouloir supprimer "${villaToDelete?.title}" ?`}
          type="danger"
...
export { VillasManagement as default };
