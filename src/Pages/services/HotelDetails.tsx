import { useParams, useNavigate } from 'react-router-dom';
import { usePropertyDetails } from '../../hooks/usePropertyDetails';
import { ArrowLeft } from 'lucide-react';

const HotelDetails = () => {
  const { id } = useParams();
  const navigate = useNavigate();
  const { property, loading, error } = usePropertyDetails('hotel', id);

  const handleViewAvailability = () => {
    if (!property || !id) return;
    navigate(`/hotels/${id}/reserver`, {
      state: {
        service: {
          id: property.id,
          title: property.title || property.name,
          description: property.description || '',
          price: property.price_per_night || property.price || 0,
          images: Array.isArray(property.images) ? property.images : [],
          type: 'hebergement'
        }
      }
    });
  };

  if (loading) {
    return (
      <div className="min-h-screen flex items-center justify-center">
        <div className="animate-spin rounded-full h-12 w-12 border-t-2 border-b-2 border-emerald-500"></div>
      </div>
    );
  }

  if (error) {
    return (
      <div className="min-h-screen flex items-center justify-center">
        <div className="text-center">
          <p className="text-red-500 mb-4">Erreur: {error.message}</p>
          <button
            onClick={() => navigate(-1)}
            className="text-emerald-600 hover:text-emerald-700"
          >
            Retour
          </button>
        </div>
      </div>
    );
  }

  if (!property) {
    return (
      <div className="min-h-screen flex items-center justify-center">
        <div className="text-center">
          <p className="text-gray-500 mb-4">Hôtel non trouvé</p>
          <button
            onClick={() => navigate(-1)}
            className="text-emerald-600 hover:text-emerald-700"
          >
            Retour
          </button>
        </div>
      </div>
    );
  }

  return (
    <div className="min-h-screen bg-gray-50">
      <div className="container mx-auto px-4 py-8">
        <button
          onClick={() => navigate(-1)}
          className="mb-6 flex items-center text-emerald-600 hover:text-emerald-700"
        >
          <ArrowLeft className="h-5 w-5 mr-2" />
          Retour
        </button>

        <div className="bg-white rounded-xl shadow-lg overflow-hidden">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 p-6">
            <div>
              {property.images && property.images.length > 0 ? (
                <img 
                  src={property.images[0]} 
                  alt={property.name || property.title}
                  className="w-full h-96 object-cover rounded-lg"
                />
              ) : (
                <div className="w-full h-96 bg-gray-200 rounded-lg flex items-center justify-center">
                  <span className="text-gray-400">Aucune image</span>
                </div>
              )}
            </div>
            
            <div>
              <h1 className="text-3xl font-bold mb-4">{property.name || property.title}</h1>
              
              {property.city && (
                <p className="text-gray-600 mb-4">
                  {property.city} {property.stars ? `• ${property.stars} étoiles` : ''}
                </p>
              )}

              <p className="text-2xl font-semibold text-emerald-600 mb-4">
                À partir de {property.price_per_night || property.price || 0} MAD / nuit
              </p>
              
              <p className="text-gray-700 mb-6">{property.description}</p>
              
              <div className="bg-gray-50 rounded-lg p-4 mb-6">
                <h2 className="font-bold mb-3 text-lg">Équipements</h2>
                <div className="grid grid-cols-2 gap-3">
                  {property.rooms && (
                    <p>Chambres: {property.rooms}</p>
                  )}
                  <p>WiFi: {property.has_wifi ? 'Gratuit' : 'Non disponible'}</p>
                  <p>Petit déjeuner: {property.has_breakfast ? 'Inclus' : 'Non inclus'}</p>
                  <p>Piscine: {property.has_pool ? 'Oui' : 'Non'}</p>
                  <p>Parking: {property.has_parking ? 'Gratuit' : 'Payant'}</p>
                  {property.capacity && (
                    <p>Capacité: {property.capacity} personnes</p>
                  )}
                </div>
                {property.amenities && Array.isArray(property.amenities) && property.amenities.length > 0 && (
                  <div className="mt-3">
                    <span className="font-medium">Autres équipements:</span>
                    <ul className="list-disc list-inside mt-1 ml-4">
                      {property.amenities.map((amenity: string, index: number) => (
                        <li key={index} className="text-sm text-gray-600">{amenity}</li>
                      ))}
                    </ul>
                  </div>
                )}
              </div>
              
              <button 
                onClick={handleViewAvailability}
                className="w-full bg-emerald-600 text-white py-3 px-6 rounded-lg hover:bg-emerald-700 transition-colors font-semibold"
              >
                Voir les disponibilités
              </button>
            </div>
          </div>
        </div>
      </div>
    </div>
  );
};

export default HotelDetails;