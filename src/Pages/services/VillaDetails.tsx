...
import { ArrowLeft, MapPin, Bed, Bath, Square, Calendar, Waves, TreePine } from 'lucide-react';

const VillaDetails = () => {
  const { id } = useParams();
  const navigate = useNavigate();
  const { property, loading, error } = usePropertyDetails('villa', id);

  const handleReserve = () => {
    if (!property) return;
    navigate(`/villas/${id}/reserver`, {
      state: {
        service: {
          id: property.id,
  ...
  if (!property) {
    return (
      <div className="min-h-screen flex items-center justify-center">
        <div className="text-center">
          <p className="text-gray-500 mb-4">Villa non trouvée</p>
          <button
            onClick={() => navigate(-1)}
            className="text-emerald-600 hover:text-emerald-700"
          >
              ...
              <div className="bg-gray-50 rounded-lg p-4 mb-6">
                ...
                <div className="grid grid-cols-2 gap-4">
                  ...
                  {property.land_area && (
                    <div className="flex items-center">
                      <Square className="h-5 w-5 mr-2 text-emerald-600" />
                      <span>Terrain: {property.land_area} m²</span>
                    </div>
                  )}
                  {property.has_pool !== undefined && (
                    <div className="flex items-center">
                      <Waves className="h-5 w-5 mr-2 text-emerald-600" />
                      <span>Piscine: {property.has_pool ? 'Oui' : 'Non'}</span>
                    </div>
                  )}
                  {property.has_garden !== undefined && (
                    <div className="flex items-center">
                      <TreePine className="h-5 w-5 mr-2 text-emerald-600" />
                      <span>Jardin: {property.has_garden ? 'Oui' : 'Non'}</span>
                    </div>
                  )}
                </div>
              </div>
